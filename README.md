...
- **TanStack Query**, le feed sort d'une fixture JSON via un mock client : [api.ts](src/features/feed/api.ts) serait le point d'entrée prévu si on ajoute un vrai backend.
...
2. **to-issues**, le plan est découpé en 11 issues, à implémenter dans l'ordre. Elles sont dans [.grilled/issues/](.grilled/issues/). Je les laisse volontairement dans le repo pour que tu puisses les lire mais en temps normal, je ne les aurais pas commitées (pas utile).
...
J'ai tendance à créer une nouvelle conversation par issue pour que l'IA reparte sur une base saine : pas de pollution des issues précédentes, et on reste sous ~250K tokens. Au-delà, je trouve qu'elle hallucine beaucoup plus.
...
- **Logo et icônes custom** : le brief ne fournissait pas le logo de bpm pour la tab bar ni la petite icône custom dans les cards de type prompt. J'ai donc recréé rapidement le logo de BPM sur Figma au format svg, il n'est donc pas parfait mais je n'allais pas t'embêter avec ça. Et pour l'icône custom de la card prompt, j'ai mis une alternative de lucide.
- **Couleurs à la pipette** (approximatives) pour celles hors palette du brief : `accentPurple`, `accentPink`, gris des panneaux.
- **Rings sport** : remplissage proportionnel à la fréquence d'entraînement (1-2 → 1/3, 3-4 → 2/3, 5+ → plein), règle inventée, je ne suis pas sûr que ce soit exactement la bonne.

  <img src="docs/readme/sport-card-rings.jpeg" width="340" alt="Rings de la carte sport">

- **Photo Allowance** : pour pouvoir voir la card « photos verrouillées » sur certains profils, j'ai dû appliquer dans le code une règle avec `VIEWER_UPLOADED_PHOTOS_COUNT = 3`.
...
- **Brancher un vrai backend** évidemment. Avec [api.ts](src/features/feed/api.ts), remplacer le mock client par de vraies requêtes, ajouter les mutations like/pass.
